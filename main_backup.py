...
import client
import socket, time, datetime, threading, os, io, csv
import pandas as pd
from socket import *
import pymssql
...
class myWindow(QMainWindow, form_class):
    def __init__(self):
        ...
        self.label_2.setStyleSheet("background-color: #CCCCCC;" "border-radius: 3px")
        self.label_5.setStyleSheet("background-color: #CCCCCC;" "border-radius: 3px")
                            #"color: blue;"
                              # "border-style: solid;"
                              # "border-width: 2px;"
                              # "border-color: #999999;"
        ...
        if self.inspectionNumber > 1 :
            ...
            self.inspectionSensor.append(self.df_tagInfo[self.df_tagInfo.columns[2]][0])
            self.inspectionSensor.append(self.df_tagInfo[self.df_tagInfo.columns[2]][1])
        else:
            self.inspectionSensor.append('onlyOne')
    ...
    def updateMsg(self, msg):
        # if self.cameraBrand =="Banner":
        #     countInfo = msg.split(',')
        #     resultFirst, resultSecond = countInfo[0:2]
        #
        ...
        # else:
        countInfo = msg.split(',')
        totalCount, passCount, failcount = countInfo[0:3]
        print(totalCount, passCount, failcount)
        self.label_23.setText(totalCount)
        self.label_21.setText(passCount)
        self.label_24.setText(failcount)

    def updateMsgSecond(self, msg):
        countInfo = msg.split(',')
        totalCount, passCount, failcount = countInfo[0:3]
        print(totalCount, passCount, failcount)
        self.label_31.setText(totalCount)
        self.label_39.setText(passCount)
        self.label_41.setText(failcount)
...
class MyThread(threading.Thread):
    ...
    def run(self):
        while not(exit.is_set()):
            ...
            resultList =[totalCount, passCount, failCount]
...
def writeResultHistory(firstHistory, secondHistory):
    script_dir = os.path.dirname(__file__)
    ...
    try:
        ...
        print('Reading Result History Error')
    else:
        try:
            wr = csv.writer(f)
            ...
            print('수집 주기 정보 저장에 실패했습니다.')
        else:
            print('수집 주기 정보 저장에 성공했습니다.')
...
def writeTimeIntervalValue():
    ...
    try:
        ...
        print('Time Interval writing Error')
    else:
        try:
            wr = csv.writer(f)
...
def readTimeIntervalValue():
    ...
    try:
        ...
        print('Time Interval Reading Error')
    else:
        rdr = csv.reader(f)
        timeIntervalValue = []
        for line in rdr:
...
def readEthernetSettingValue():
    ...
    try:
        ...
        print('카메라 설정 파일이 경로에 존재하지 않습니다.')
    else:
        rdr = csv.reader(f)
        settingValueList = []
        for line in rdr:
    ...
    def writeEthernetSettingValue(self):
        ...
        try:
            ...
            try:
                ...
                print('카메라 설정 정보 저장에 실패했습니다.')
            else:
                print('카메라 설정 정보 저장에 성공했습니다.')
...
class dbDialog(QtWidgets.QDialog):
    ...
    def writeSettingValue(self):
        ...
        try:
            ...
            print('DB 접속 설정 파일이 경로에 존재하지 않습니다.')
        else:
            try:
                wr = csv.writer(f)
                ...
                print('DB접속 설정 정보 저장에 실패했습니다.')
            else:
                print('DB접속 설정 정보 저장에 성공했습니다.')
